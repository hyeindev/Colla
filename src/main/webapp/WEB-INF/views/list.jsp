   ...
   var lTitle2;
	                        ...
	                        .click(
	                              function() {
	                                 ...
	                                 $("#compareTitle" + data.lFk)
	                                       .empty();
	                                 var formtag = 
	                                       "<input id='lFk2' type='hidden' name='lFk' value='" + data.lFk + "' />"
	                                       + "<input id='bFk2' type='hidden' name='bFk' value='${bFk}' />"
	                                       + "<input id='lTitle2' type='text' name='lTitle' placeholder='Create Title' class='enterTitle'/>&nbsp;"
	                                       + "<button class='submitTitle' onclick='updateTitle();'>send</button>";

	  	                                 $("#compareTitle" + data.lFk)
	                                       .append(formtag);


	  	                       		 lFk2 = parseInt($("#lFk2").val());
	  	                       		 bFk2 = parseInt($("#bFk2").val());

	                              });

	                  // "<form action='/list/updateListTitle' method='post'>"
	                  // "</form>"
	               },
	               error : function(xhr, status, data) {
	                  console.log(error);
	   ...
	   $(function() {
		...
		function updateTitle() {
				...
				$.ajax({
								url: "/list/updateListTitle?lFk=" + lFk2 + "&bFk=" + bFk2 + "&lTitle=" + lTitle2,
								type:"post",
								 beforeSend : function(xhr) {
					                  xhr.setRequestHeader("${_csrf.headerName}","${_csrf.token}");
					               },
								success: function() {
											$("#compareTitle"+ lFk2).empty();
											$("#compareTitle" + lFk2)
		                                       .append(""+lTitle2);
